...
fun dbSetupLarge(args: List<String>) {
    val enforcer = getEnforcer()
//    val names = getNames()
    val names = listOf("ale")
    val roles = getRoles()
//    val domains = getDomains()
    val domains = listOf("domainAle")
    val pattern = """(?i)\d""".toRegex()
//    val resources = getResources().map {listOf(it, it.take(pattern.find(it)?.range?.first ?: it.length))}
    val nResources = if (args.size == 1) args[0].toInt() else 1000
    val resources = (1..nResources).map { "course".plus(it.toString()) }.toList()
    val userRoles =
        generateSequence {
            listOf(
...
//            val name = names.shuffled().first()
            ...
            val sub = CustomSubject(
                name = name,
                isAdmin = isAdmin,
                age = 92,
                address = "Via Borromini"
